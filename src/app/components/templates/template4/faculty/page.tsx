    ...
    >
      <div className="max-w-6xl mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-20">
          ...
          <h1 className="text-[32px] md:text-[36px] font-serif font-medium text-[#E86A58] mb-6 leading-tight tracking-tight">
            Mentors & Guides
          </h1>
          
          <p className="text-[17px] text-[#5A5A5A] max-w-2xl mx-auto leading-[1.7] px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          {facultyMembers.map((faculty, index) => {
            ...
            const cardStyle = index % 3 === 0 ? "border-t-[3px] border-[#E86A58]" : 
                            index % 3 === 1 ? "border-l-[3px] border-[#E86A58]" : 
                            "border-b-[3px] border-[#E86A58]";
                ...
                <div className="relative px-6">
                  ...
                  <div className={`relative mb-6 ${textAlignment === 'text-center' ? 'flex justify-center' : ''}`}>
                    ...
                    <div className="absolute -top-2 -right-2 w-10 h-10 rounded-full bg-white border border-[#EDEDED] flex items-center justify-center shadow-sm">
                      <PersonalIcon className="text-[#E86A58] text-sm" />
                    </div>

                    {/* Experience - organic placement */}
                    <div className={`mt-4 ${textAlignment === 'text-center' ? 'mx-auto' : ''}`}>
                      <div className="inline-block px-3 py-1.5 bg-[#FAFAFA] border border-[#EDEDED] rounded-sm text-[13px] text-[#5A5A5A]">
                        {faculty.experience}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    <h3 className="text-[18px] font-serif font-medium text-[#1E1E1E] mb-2">
                      {faculty.name}
                    </h3>
                    <div className="mb-4">
                      <p className="text-[14px] text-[#7A7A7A] font-medium mb-1">
                        {faculty.designation}
                      </p>
                  ...
                  <div className="mb-6">
                    <p className="text-[15px] text-[#5A5A5A] leading-[1.6] mb-3">
                      {faculty.description}
                    </p>
                    {faculty.bio && (
                      <p className="text-[14px] text-[#7A7A7A] leading-[1.6]">
                        {faculty.bio}
                      </p>
                    )}
                  </div>
                    ...
                    <div className="mb-6">
                      <div className={`flex flex-wrap gap-2 ${textAlignment === 'text-center' ? 'justify-center' : ''}`}>
                        {faculty.expertise.slice(0, 3).map((skill, idx) => (
                          <span
                            key={idx}
                            className="px-3 py-1.5 bg-[#FAFAFA] text-[#5A5A5A] text-[12px] rounded-sm border border-[#EDEDED]"
                          >
                            {skill}
                          </span>
                        ))}
                  ...
                  <div className={`space-y-3 ${textAlignment === 'text-center' ? 'text-center' : ''}`}>
                    {faculty.linkedin && (
                      <a
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 text-[13px] text-[#5A5A5A] hover:text-[#E86A58] transition-colors duration-200"
                      >
          ...
          <div className="bg-white border border-[#EDEDED] rounded-sm p-8 md:p-12">
            <div className="text-center mb-8">
              <div className="inline-flex items-center gap-3 mb-6">
                ...
                <FaChalkboardTeacher className="text-[#8A8A8A] text-lg" />
                <div className="w-6 h-px bg-[#E0E0E0]"></div>
              </div>
        ...
        <div className="text-center">
          ...
          <p className="text-[15px] text-[#5A5A5A] max-w-xl mx-auto leading-relaxed">
